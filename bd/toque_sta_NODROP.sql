...
-- -----------------------------------------------------
-- Table `toque_sta`.`delivery`
-- -----------------------------------------------------
CREATE  TABLE IF NOT EXISTS `toque_sta`.`delivery` (
  `id_delivery` INT NOT NULL AUTO_INCREMENT ,
  `repair_id` INT NOT NULL ,
  PRIMARY KEY (`id_delivery`) ,
  INDEX `fk_delivery_repair1` (`repair_id` ASC) ,
  CONSTRAINT `fk_delivery_repair1`
    FOREIGN KEY (`repair_id` )
    REFERENCES `toque_sta`.`repair` (`id_repair` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
